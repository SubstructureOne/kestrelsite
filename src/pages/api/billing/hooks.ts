...
export default async function handler(
    ...
    if (
        ...
        if (userId === null) {
            const error =
                "Cannot process external transaction because userId is missing from metadata";
            logger.error(error);
        ...
        if (amount_cents == null) {
            const error =
                "Cannot process external transaction: subtotal not specified";
            logger.error(error);
            res.status(400).json({ error });
            return;
        }
        if (checkoutSession.payment_status == "paid") {
            const newTxn: NewExternalTransactionInfo = {
                ...
                exttxn_time: new Date(
                    1000 * checkoutSession.created,
                ).toISOString(),
            };
            ...
            logger.info(
                `Checkout session ${checkoutSession.id} completed but payment not completed; awaiting`,
            );
        }
    } else if (event.type == "checkout.session.async_payment_failed") {
        const checkoutSession = event.data.object;
        logger.warn(
            `Payment failed for checkout session ${checkoutSession.id} (${checkoutSession.amount_subtotal} from ${checkoutSession.client_reference_id}`,
        );
    }
