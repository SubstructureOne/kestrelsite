...
export async function getuser(
    ...
    if (result.rows.length > 0) {
        ...
        if (pgPassword.isErr) {
            return Err({
                friendly: "Could not decrypt postgres password",
                reason: pgPassword,
            });
        }
        return Ok({
...
export async function getExternalTransactions(
    client: Client,
    userId: string,
): Promise<ExternalTransactionInfo[]> {
    const result = await client.query(
        `
...
export async function createExternalTransaction(
    client: Client,
    exttxn: NewExternalTransactionInfo,
): Promise<number> {
    const result = await client.query({
...
export async function createUser(
    ...
    if (encryptedPassword.isErr) {
        ...
        return Err({
            friendly: "Couldn't encrypt Postgres password",
            cause: encryptedPassword,
        });
    }
    let result;
    try {
        ...
        return Err({
            friendly: "Couldn't create user row",
